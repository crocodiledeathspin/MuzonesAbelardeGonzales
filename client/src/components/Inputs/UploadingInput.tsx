...
const onDrop = useCallback((acceptedFiles: File[]) => {
    ...
    // Handle initial value and existing image URL
    useEffect(() => {
        if (value) {
            ...
            // For existing image URL (already stored on server)
    ...
    return (
        <>
            ...
            <div className={`transition border border-gray-300 border-dashed cursor-pointer rounded-lg hover:border-blue-900 ${errors ? 'mb-0' : 'mb-4'}`}>
                <div {...getRootProps()} className={`rounded-lg border-dashed border-gray-300 p-7 lg:p-10 ${isDragActive ? "border-blue-600 bg-gray-100" : "border-gray-300 bg-gray-50"}`}>
                    <input {...getInputProps()} name={name} id={name} />
                    <div className="flex flex-col items-center m-0">
                        {preview ? (
                            <img src={preview} alt="Profile Picture Preview" className="object-cover rounded-full w-[185px] h-[185px]" />
                        ) : (
                            <>
